"""Creative QA. The copywriter agent is asked to respect lengths, avoid claims the advertiser
never made, and avoid what the persona is disinterested in; this module checks that it did.
Hard issues trigger one regeneration with the issues quoted back, soft issues become warnings.

Rules are small classes behind one Protocol so the interview's "add a lint rule" is one class
and one line in DEFAULT_RULES."""
...
class LintContext:
    ...
    description: str                  # the advertiser text: stated claims are allowed
    other_headlines: tuple[str, ...]  # headlines already written for other personas
...
class LengthRule:
    ...
    def check(self, ctx: LintContext) -> list[LintIssue]:
        limits = (("headline", ctx.draft.headline, HEADLINE_MAX),
                  ("body", ctx.draft.body, BODY_MAX),
                  ("cta", ctx.draft.cta, CTA_MAX))
        issues = []
        for field_name, text, limit in limits:
            if not text.strip():
                issues.append(_hard(self.name, f"{field_name} is empty"))
            elif len(text) > limit:
                issues.append(_hard(self.name, f"{field_name} is {len(text)} characters "
                                               f"(max {limit})"))
        return issues
...
class ClaimsRule:
    ...
    def check(self, ctx: LintContext) -> list[LintIssue]:
        ...
        return [_hard(self.name, f"unsubstantiated claim '{m.group(0)}'")
                for m in self._pattern.finditer(text) if m.group(0).lower() not in stated]
...
class PersonaDisinterestRule:
    ...
    # a vague eco word is fine when a specific claim sits next to it
    _specific_eco = re.compile(r"\b(recycled|refill\w*|certified|carbon|plastic|%|compostable|"
                               r"b corp|fsc|organic)\b", re.I)
    _gift_framing = re.compile(r"\b(gift|gifting|give|treat someone)\b", re.I)

    def check(self, ctx: LintContext) -> list[LintIssue]:
        text = f"{ctx.draft.headline} {ctx.draft.body} {ctx.draft.cta}"
        issues = []
        for phrase in ctx.persona.disinterested_in:
            pattern = self._patterns.get(phrase.lower())
            if pattern is None:
                continue
            match = pattern.search(text)
            if match is None:
                continue
            if phrase == "vague eco claims" and self._specific_eco.search(text):
                continue
            if phrase == "subscription-only" and self._gift_framing.search(text):
                continue
            issues.append(_hard(self.name, f"'{match.group(0)}' trips {ctx.persona.name}'s "
                                            f"disinterest in {phrase}"))
        return issues


class DuplicateHeadlineRule:
    name = "duplicate_headline"

    def check(self, ctx: LintContext) -> list[LintIssue]:
        mine = ctx.draft.headline.strip().lower()
        if any(mine == other.strip().lower() for other in ctx.other_headlines):
            return [_soft(self.name, "headline duplicates another variant")]
        return []
...
class StyleRule:
    ...
    def check(self, ctx: LintContext) -> list[LintIssue]:
        ...
        issues = []
        if len(self._caps_word.findall(text)) > 1:
            issues.append(_soft(self.name, "more than one ALL-CAPS word"))
        if text.count("!") > 1:
            issues.append(_soft(self.name, "more than one exclamation mark"))
        return issues


DEFAULT_RULES: tuple[LintRule, ...] = (
    LengthRule(), ClaimsRule(), PersonaDisinterestRule(), DuplicateHeadlineRule(), StyleRule(),
)
...
def _hard(rule: str, message: str) -> LintIssue:
    return LintIssue(severity=LintSeverity.HARD, rule=rule, message=message)


def _soft(rule: str, message: str) -> LintIssue:
